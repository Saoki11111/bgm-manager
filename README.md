...
- 小さなストリーミングバッファを調整する。現在は`mpv`の`--cache=yes --cache-secs=30`で短く先読みしています
- 再生が止まる時の切り分け用に、アプリ版は`~/Library/Logs/BGMManager/mpv.log`へ最後の再生ログを残します
- 任意でローカルキャッシュを使う。再生開始や一時的な回線不安定には効く可能性があります
- ただし永続キャッシュはデフォルトにしない方針です。ディスク容量を使い、URLや音源が個人データ化し、YouTube側のURL期限切れで再利用できない場合があります
- 曲ごとの並び替えをメニューからできるようにする
- `mpv`や`yt-dlp`が見つからない時に、アプリ内で原因とインストール方法を表示する
- 起動時にメニューバーへ常駐し、Dockに置いても操作の中心はメニューバーに保つ

## 見直し候補

- CLIは開発用に残すか、READMEでは目立たせすぎない
- 長い時間指定を増やしすぎない。30分、1時間、2時間、3時間程度に留める
- 常時表示の情報量を増やさない。総時間や秒単位の進捗はメニュー内に置く
- 複雑なプレイリスト管理を入れすぎない。曲を追加して選ぶ、表示名を変える、削除する、ランダム、前後移動くらいに留める
